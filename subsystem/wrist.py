...
class Wrist(Subsystem):
    ...
    def init(self):
        ...
        self.wrist_angle_pub = self.table.getDoubleTopic("wrist angle").publish() 
        self.target_angle_pub = self.table.getDoubleTopic("target angle").publish()
        self.wrist_angle_moving_pub = self.table.getBooleanTopic("target angle moving").publish()
        ...
        self.wrist_applied_out_pub = self.table.getDoubleTopic("wrist applied output").publish()
        self.wrist_current_pub = self.table.getDoubleTopic("wrist current").publish()
        self.coral_in_feed_pub = self.table.getDoubleTopic("coral in feed").publish()
        self.wrist_angle_diff_pub = self.table.getDoubleTopic("wrist angle difference")
        self.algae_in_wrist_pub = self.table.getBooleanTopic("algae in wrist").publish()
        self.algae_motor_current_pub = self.table.getDoubleTopic("algae motor current").publish()
    ...
    def update_table(self) -> None:
        ...
        self.wrist_abs_ang_pub.set((math.degrees(self.encoder.get_absolute_position().value - config.wrist_encoder_zero)
            ...
            * 2
            * math.pi))
        self.calc_kg_pub.set(config.wrist_max_ff * math.cos(self.get_wrist_angle() - config.wrist_ff_offset))
        self.wrist_applied_out_pub.set(self.wrist_motor.get_applied_output())
        self.wrist_current_pub.set(self.wrist_motor.get_motor_current())
        self.coral_in_feed_pub.set(self.coral_in_feed)
        self.wrist_angle_diff_pub.set((math.degrees(self.encoder.get_absolute_position().value - config.wrist_encoder_zero)                   
            / constants.wrist_encoder_gear_ratio
            * 2
            * math.pi)-self.get_wrist_angle())
        self.algae_in_wrist_pub.set(self.algae_in_wrist)
        self.algae_motor_current_pub.set(self.algae_motor.get_motor_current())
